$(document).ready(function() {
    $(".pass-input-holder").focusin(function() {
        if ($(this).val() === "请输入TA的密语") {
            $(this).val("");
        }
    }).focusout(function() {
        if ($(this).val() === "") {
            $(this).val("请输入TA的密语");
        }
    });

    $(".cont-text-area").focusin(function() {
        if ($(this).val() === "写下你想说的话吧") {
            $(this).val("");
        }
    }).focusout(function() {
        if ($(this).val() === "") {
            $(this).val("写下你想说的话吧");
        }
    });

    $(".set-input-holder").focusin(function() {
        if ($(this).val() === "请设置一个密语吧" || $(this).val() === "密语已被使用过了" || $(this).val() === "密语格式错误" || $(this).val() === "出现未知错误") {
            $(this).val("");
        }
    }).focusout(function() {
        if ($(this).val() === "") {
            $(this).val("请设置一个密语吧");
            $("#set-pw-hint").html("");
        } else {
            $("#set-pw-hint").html('<img src="resource/public/loading.gif" style="height: 80%;">');
            let res = validate();
            if (res < 0) {
                $(this).val("出现未知错误");
                $("#set-pw-hint").html('<img src="resource/dialog-speak/public/fault.png" style="height: 80%;">');
            } else {
                if (res === 1) {
                    $(this).val("密语已被使用过了");
                } else {
                    $("#set-pw-hint").html('<img src="resource/dialog-speak/public/ok.png" style="height: 80%;">');
                }
            }
        }
    });

    $("input").focusin(function() {
        inputFocus = true;
        if (detectorDisplayed) {
            $("#screen-direction-detector").fadeOut();
            detectorDisplayed = false;
        }
    }).focusout(function() {
        inputFocus = false;
    });

    $("textarea").focusin(function() {
        inputFocus = true;
        if (detectorDisplayed) {
            $("#screen-direction-detector").fadeOut();
            detectorDisplayed = false;
        }
    }).focusout(function() {
        inputFocus = false;
    });

});